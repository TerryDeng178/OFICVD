# -*- coding: utf-8 -*-
"""TestnetAdapter Implementation

测试网适配器：使用Binance Testnet API
"""
...
class TestnetAdapter(BaseAdapter):
    ...
    使用Binance Testnet API或Broker Gateway MCP
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if use_broker_gateway:
            ...
            broker_cfg_with_mock["mock_enabled"] = broker_cfg.get("mock_enabled", True)  # 测试网默认启用Mock
            broker_cfg_with_mock["mock_output_path"] = str(self.output_dir / "testnet_orders.jsonl")
            ...
            self.binance_api = BinanceFuturesAPI(api_key=api_key, secret_key=secret_key, testnet=True)
    ...
    def _load_rules_impl(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 尝试从 Broker Gateway 或 Binance API 获取最新价格
            ...
            if mark_price and mark_price > 0:
                ...
                # 使用一个合理的默认价格（BTCUSDT 约 50000）
                default_mark_price = 50000.0 if "BTC" in symbol else 1000.0
                default_rules["mark_price"] = default_mark_price
                logger.warning(f"[TestnetAdapter] Using fallback mark_price={default_mark_price} for {symbol}")
            
            logger.debug(f"[TestnetAdapter] Loaded rules for {symbol}, mark_price={default_rules.get('mark_price')}")
            return default_rules
            
        except Exception as e:
            logger.error(f"[TestnetAdapter] Failed to load rules for {symbol}: {e}")
            # 返回默认规则
            fallback_rules = {
                "qty_step": 0.0001,
                "qty_min": 0.001,
        ...
        try:
            ...
            if self.broker_client:
                try:
                    ...
                    logger.debug(f"[TestnetAdapter] Broker client ticker failed: {e}")
            ...
            logger.debug(f"[TestnetAdapter] Failed to get mark_price for {symbol}: {e}")
            return None
    ...
    def _submit_impl(self, order: AdapterOrder) -> AdapterResp:
        ...
        try:
            ...
            logger.error(f"[TestnetAdapter] Submit failed: {e}")
    ...
    def _cancel_impl(self, symbol: str, broker_order_id: str) -> AdapterResp:
        ...
        try:
            ...
            logger.error(f"[TestnetAdapter] Cancel failed: {e}")
    ...
    def fetch_fills(self, symbol: str, since_ts_ms: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[TestnetAdapter] Fetch fills failed: {e}")
            return []
